...
var HlsQualitySelectorPlugin = function () {
  ...
  HlsQualitySelectorPlugin.prototype.onAddQualityLevel = function onAddQualityLevel() {
    ...
    var _loop = function _loop(i) {
      if (!levelItems.filter(function (_existingItem) {
        return _existingItem.item && _existingItem.item.value === levels[i].bitrate;
      }).length) {
        var levelItem = _this.getQualityMenuItem.call(_this, {
          label: levels[i].bitrate + 'bps',
          value: levels[i].bitrate
        });
  ...
  /**
   * Sets quality (based on media bitrate)
   *
   * @param {number} bitrate - A number representing HLS playlist.
   */


  HlsQualitySelectorPlugin.prototype.setQuality = function setQuality(bitrate) {
    var qualityList = this.player.qualityLevels();

    for (var i = 0; i < qualityList.length; ++i) {
      var quality = qualityList[i];

      quality.enabled = quality.bitrate === bitrate || bitrate === 'auto';
    }
